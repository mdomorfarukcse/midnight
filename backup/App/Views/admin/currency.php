...
$currency = $container->has('detailId') ? (new Currency())->find($container->get('detailId')) : new Currency();

if ($request->isMethod('post')) {

    try {

        $new = empty($currency->getId());

        $currency
            ->setCode($request->request->get('code'))
            ->setName($request->request->get('name'))
            ->setSymbol($request->request->get('symbol'))
            ->setStatus(($request->request->has('status') && $request->request->get('status') == 'on') ? 1 : 0);

        $currency->store();

        $session->getFlashBag()->add('success', 'Success');

        if ($new) header('location: /admin/currency/detail/' . $currency->getId() . '?confirm_message=Success');
...
<html lang="en">
<head>
    <title><?= $language::translate('Currency') ?> - <?= SITE_NAME ?> </title>
    <?php include("css.php")?>
        ...
        <div class="layout-px-spacing">
            <div class="row layout-top-spacing">
                <div class="col-xl-6 col-lg-6 col-sm-12 ">
                    <div class="card">
                        <div class="card-body">
                            <h5 class="card-title"><?= $language::translate('Currency') ?></h5>
                            <form action="" method="post" style="width: 100%">
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-10">
                                        <input type="text" name="code" class="form-control" id="code" value="<?= $currency->getCode() ?>" required>
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <label for="name" class="col-sm-2 col-form-label"> <?= $language::translate('Name') ?></label>
                                    <div class="col-sm-10">
                                        <input type="text" name="name" class="form-control" id="name" value="<?= $currency->getName() ?>" required>
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <label for="symbol" class="col-sm-2 col-form-label"> <?= $language::translate('Symbol') ?></label>
                                    <div class="col-sm-10">
                                        <input type="text" name="symbol" class="form-control" id="symbol" value="<?= $currency->getSymbol() ?>" required>
                                    </div>
                                </div>
                                <div  class="form-row">
                                    ...
                                    <div  style="    margin-bottom: 0;" class="form-group col-md-10  col-6 ">
                                        <label class="switch s-icons s-outline  s-outline-primary">
                                            <input type="checkbox" name="status" <?= ($currency->getStatus()) ? 'checked' : '' ?>>
