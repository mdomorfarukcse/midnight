...
if($container->has('detailId')) {
    ...
    $brands = (new Category())->findBy(['filter' => ['parent_id' => $brand->getParent()->getParent()->getParent()->getParentId(), 'is_active' => 1, 'type' => 'brand']]);
    $models = (new Category())->findBy(['filter' => ['parent_id' => $brand->getParent()->getParent()->getParentId(), 'is_active' => 1, 'type' => 'model']]);
    $generations = (new Category())->findBy(['filter' => ['parent_id' => $brand->getParent()->getParentId(), 'is_active' => 1, 'type' => 'generation']]);
}
...
if ($request->isMethod('post')) {
    ...
    try {
        ...
        if ($new) header('location: /admin/vehicle/engine/detail/' . $brand->getId() . '?confirm_message=Success');
...
<html lang="en">
<head>
    <title><?= $language::translate('Engine') ?> - <?= SITE_NAME ?> </title>
    <?php include("css.php")?>
                        ...
                        <div class="card-body">
                            ...
                            <form action="" method="post" enctype="multipart/form-data" style="width: 100%">
                                    ...
                                    <div class="col-sm-10">
                                        <select name="parent_id" id="status" class="form-control" onchange="getSubCategoriesForSelect('category', this)" style="width: 50%">
                                            ...
                                            foreach ($types as $key => $type) {
                                                if($container->has('detailId')) {?>
                                                    <option value="<?= $type->getId() ?>"<?= ($type->getId() == $brand->getParent()->getParent()->getParent()->getParentId() ? 'selected' : '') ?>><?= $type->getName() ?></option>
                                                <?php } else { ?>
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-10">
                                        <select name="parent_id" id="models" class="form-control" onchange="getSubCategoriesForSelect('model', this)" style="width: 50%">
                                ...
                                <div class="form-group row">
                                    <label for="parent_id" class="col-sm-2 col-form-label"> <?= $language::translate('Generation') ?></label>
                                    <div class="col-sm-10">
                                        <select name="parent_id" id="generations" class="form-control" onchange="getSubCategoriesForSelect('year', this)" style="width: 50%">
                                            <option value=""><?= $language::translate('Select Generation') ?></option>
                                            <?php
                                            if($container->has('detailId')) {
                                                /* @var Category $type */
                                                foreach ($generations as $key => $type) {?>
...
<script>
    ...
    function pushAfterSelectCategory(type, result)
    {
        ...
        if(type == 'model') {
            ...
            var selectOptionsHtml = '<option value="">Select Generation</option>';

            result.forEach(function (data) {
                selectOptionsHtml += '<option value="' + data.id + '">' + data.name + '</option>\n'
            });

            $('#generations').html(selectOptionsHtml);
        }
    }
