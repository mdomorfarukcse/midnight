...
$category = $container->has('detailId') ? (new Category())->find($container->get('detailId')) : new Category();

$types = (new Category())->findBy(['filter' => ['parent_id' => 0]]);
...
if ($request->isMethod('post')) {
    ...
    if (!empty($image = $request->files->get('image'))) {
        ...
        $_image = $image->move($_SERVER['DOCUMENT_ROOT'] . '/icon/category/', 'brand-' . $sef . '-' . time() .'.' . $image->getClientOriginalExtension());
        $category->setImage($_image->getBasename());
        $category->setIcon($_image->getBasename());
    }

    try {

        $new = empty($category->getId());
        ...
        if ($new) header('location: /admin/vehicle/category/detail/' . $category->getId() . '?confirm_message=Success');
...
<html lang="en">
<head>
    <title><?= $language::translate('Adding a Category') ?> - <?= SITE_NAME ?> </title>
    <?php include("css.php")?>
        ...
        <div class="layout-px-spacing">
            ...
            <div class="row layout-top-spacing">
                <div class="col-xl-8 col-lg-8 col-sm-12 ">
                    <div class="card">
                        <div class="card-body">
                            <h5 class="card-title"><?= $language::translate('Adding a Category') ?></h5>
                            <form action="" method="post" enctype="multipart/form-data" style="width: 100%">
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-10">
                                        <input type="text" name="name" class="form-control" id="name" value="<?= $category->getName() ?>">
                                    </div>
                                </div>
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-10">
                                        <div class="form-check">
                                            <label class="checkbox-inline">
                                                <input type="checkbox" value="1"  name="status" <?= !$category->getStatus() ?: 'checked'; ?> data-toggle="toggle">
                                            </label>
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <label for="image" class="col-sm-2 col-form-label"> <?= $language::translate('Image') ?></label>
                                    <div class="col-sm-10">
                                        ...
                                        <img id="img-upload"src="<?= $category->getIcon(true) ?>" style="width: 100px"/>
                                    </div>
                                </div>
