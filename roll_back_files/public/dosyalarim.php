                    ...
                    <div class="col-xl-12 col-lg-12 col-sm-12  layout-spacing">
                        <div class="widget widget-table-one">
							...
							<div class="row mb-3">
								...
								<div class="col-md-6">
									<a href = "/panel/file-upload" class="btn btn-aqua bg-aqua pull-right createBtn">Create New</a>
								</div>
							</div>
                            <table id="customer-vehicle-list-datatable" class="table table-striped mt-3" style="width:100%"></table>
                        </div>
                    </div>
                </div>
            </div>
        ...
        function getCustomerVehicleListData() {
            ...
            $('#customer-vehicle-list-datatable').dataTable( {
                ...
                language : {
                    ...
                    sInfoPostFix:    "",
                    sInfoThousands:  ".",
                    sLengthMenu:     "<?= $language::translate('Show _MENU_ entries') ?>",
                ...
                rowCallback: function( row, data, index ) {
                 ...
                 if (data.status === 'awaiting_payment')
                    $('span', row).addClass('badge bg-yellow text-capitalize');
                 if (data.status === 'completed')
                    $('span', row).addClass('badge bg-success text-capitalize');
                 if (data.status === 'pending')
                    $('span', row).addClass('badge bg-aqua text-capitalize');
                 if (data.status === 'process')
                    $('span', row).addClass('badge bg-yellow text-capitalize');
                 if (data.status === 'cancel')
                    $('span', row).addClass('badge bg-red text-capitalize');
...

                  {
...

                    {
                        ...
                        render : function (data, type, row) {
                            return '<a href="/panel/my-files/detail/'+row.id+'" data-id="'+row.id+'" class="btn btn-sq btn-success detail-btn"><i class="ti-eye"></i> </a> ';
                        }
                    }
                ]
            });
