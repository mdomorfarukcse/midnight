...
<html lang="en">
<head>
    <title><?= $language::translate('Vehicle Tuning Options') ?> - <?= SITE_NAME ?> </title>
    <?php include("css.php")?>
...
<div class="main-container" id="container">
                        ...
                        <div class="col-xl-6 col-lg-6 col-sm-12 pt-3">
                            <form action="" method="get">
                                <div class="form-group">
                                    ...
                                    <select class="form-control" id="vehicle-select" name="vehicle_id" onchange="getVehicleTuningOptionListData()"></select>
                                </div>
                            </form>
                        </div>
                        <table id="vehicle-tuning-options-list-datatable" class="table table-striped" style="width:100%"></table>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        $(function () {
            ...
            if (getVehicleTuningOptionListDataTable().length > 0) getVehicleTuningOptionListData();
        });

        function getVehicleTuningOptionListData() {

            var vehicleTuningOptionDataTable = $('#vehicle-tuning-options-list-datatable').dataTable( {
                processing: true,
                serverSide: true,
                destroy : true,
                ...
                columns: [
                    {
                        data : "additional_option_name",
                        title : "<?= $language::translate('Option Name') ?>",
                        className : "dt-center",
                        orderable: false,
                        render : function (additional_option_name) {
                            return additional_option_name;
                        }
                    },
                    {
                        data : "vehicle_tuning_id",
                        title : "<?= $language::translate('Tuning') ?>",
                        className : "dt-center",
                        orderable: false,
                        render : function (vehicle_tuning_id, type, row) {
                ...
                ajax: {
                    type: "POST",
                    url: "/ajax/admin/vehicle/tuning/option/list-for-datatable",
                    data: function (data) {

                        var settings = $("#vehicle-tuning-options-list-datatable").dataTable().fnSettings();
                        var filter = {};
        ...
        function getVehicleTuningOptionListDataTable() { return $('#vehicle-tuning-options-list-datatable'); }
    </script>
    <style>
